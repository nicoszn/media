...
// ─── ResultPanel ──────────────────────────────────────────────────────────────
//
// Standard single-output results render an inline preview (video/audio player)
// directly above the download button, so the user can confirm the operation
// worked before chaining another one on top of it.
...
function ResultPanel({ result }: { result: ProcessResult }) {
  ...
  // Standard single-output result — inline preview + download
  if ("outputUrl" in result && result.outputUrl) {
    const isVideo = !result.outputName?.match(/\.(mp3|wav|aac|ogg|flac)$/i);
    return (
      <div style={{
        width: "100%", maxWidth: "560px", background: "rgba(16,185,129,0.08)",
        ...
        {/* Inline preview of the output — lets the user confirm before chaining another op */}
        ...
        <div style={{
          ...
          {!mediaFile ? (
            ...
            <>
              ...
              {/* Result (includes inline preview for standard ops) */}
