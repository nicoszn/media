...
import {
  ...
  AlertCircle, Loader2, CheckCircle2, Terminal,
  ChevronDown, ChevronUp, Undo2, History as HistoryIcon,
} from "lucide-react";
...
const OP_SUFFIXES: Record<string, string[]> = {
  ...
  split:          ["split", "parts", "segments", "divided"],
  compress:       ["compressed", "small", "lite", "compact"],
  speed:          ["speed", "fast", "sped", "adjusted"],
  loop:           ["loop", "looped", "repeat", "cycled"],
  fps:            ["fps", "framerate", "resampled", "smooth"],
  flip:           ["flipped", "mirrored", "reversed-x", "reflected"],
  reverse:        ["reversed", "backwards", "rewound", "rewind"],
  extract_audio:  ["audio", "sound", "track", "extracted"],
  extract_frames: ["frames", "stills", "shots", "captures"],
  aspect_ratio:   ["aspect", "reframed", "padded", "fitted"],
  format_convert: ["converted", "exported", "output", "reformatted"],
  volume:         ["volume", "boosted", "quieter", "adjusted"],
  rotate:         ["rotated", "turned", "oriented", "spun"],
  resize:         ["resized", "scaled", "smaller", "cropped"],
  denoise:        ["denoised", "clean", "filtered", "smooth"],
...
function deriveOutputName(originalName: string, op: string): string {
  ...
  const ext  = originalName.match(/\.([^.]+)$/)?.[1] ?? "mp4";
  const key  = `${base}::${op}`;
  const n    = exportCounters.get(key) ?? 0;
  exportCounters.set(key, n + 1);
  const suffixes = OP_SUFFIXES[op] ?? ["output"];
...
const OP_LABELS: Record<string, string> = {
  trim: "Trim", split: "Split", compress: "Compress", resize: "Resize",
  speed: "Speed", loop: "Loop", fps: "Frame Rate", flip: "Flip",
  reverse: "Reverse", extract_audio: "Extract Audio",
  extract_frames: "Extract Frames", aspect_ratio: "Aspect Ratio",
  format_convert: "Convert", volume: "Volume", rotate: "Rotate",
  denoise: "Denoise",
};
...
function DropZone({ onFile }: { onFile: (f: File) => void }) {
  ...
  return (
    <div
      role="button"
      tabIndex={0}
      aria-label="Drop a media file here or click to browse"
    ...
    >
      ...
      <div style={{ textAlign: "center" }}>
        <div style={{
          ...
          fontSize: "15px", fontWeight: 600, color: "#F1F5F9", marginBottom: "6px",
        }}>
          Drop a file here
        </div>
    ...
    <div style={{
      ...
      <button
        ...
        style={{
          ...
          borderRadius: "8px",
          color: canUndo ? "#EF4444" : "var(--color-text-muted)",
...
function SegmentPicker({ entry, onSelect }: {
  ...
  return (
    <div
      role="group"
      aria-label="Select a segment to work with"
      style={{
        width: "100%", maxWidth: "560px",
...
function ResultPanel({ result, sourceFileName, activeOp }: {
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
          ...
          <span style={{ fontFamily: "Space Grotesk, sans-serif", fontSize: "14px", fontWeight: 600, color: "#10B981" }}>
            {result.segments.length} segments · {formatBytes(result.outputSize)}
          </span>
        </div>
        <p style={{ fontFamily: "Inter, sans-serif", fontSize: "11px", color: "var(--color-text-muted)", margin: "0 0 12px", lineHeight: 1.6 }}>
          Segment 1 is active. Select any segment above to preview or continue editing it.
        </p>
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {result.segments.map((seg, i) => {
            const hue = 260 + i * 22;
            const name = deriveOutputName(sourceFileName, "split");
            return (
              <div key={i} style={{
                ...
                <a href={seg.url} download={name} style={dlStyle}>
                  <Download size={13} />
                </a>
              </div>
            );
            ...
            <a key={i} href={url} download={`frame_${String(i + 1).padStart(4, "0")}.jpg`}>
              <img
                ...
                style={{
                  ...
                  borderRadius: "6px", border: "1px solid var(--color-border)", display: "block",
                }}
              />
            </a>
          ))}
  ...
  if ("outputUrl" in result && result.outputUrl) {
    ...
    const outputName = deriveOutputName(sourceFileName, activeOp);
    return (
      <div style={{
        width: "100%", maxWidth: "560px",
        background: "rgba(16,185,129,0.08)",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: "Space Grotesk, sans-serif", fontSize: "13px", fontWeight: 600, color: "#10B981" }}>
              Done
            </div>
            <div style={{
              ...
              {outputName}{result.outputSize ? ` · ${formatBytes(result.outputSize)}` : ""}
            </div>
          </div>
          <a href={result.outputUrl} download={outputName} style={dlStyle}>
        ...
        <p style={{ fontFamily: "Inter, sans-serif", fontSize: "11px", color: "var(--color-text-muted)", margin: 0 }}>
          Result is now the active file — run another operation on it, or Undo to go back.
        </p>
      </div>
    );
  }
...
export default function EditorShell({ lockedOp, defaultFormat }: EditorShellProps) {
  ...
  const {
    mediaFile, history, activeEntry, canUndo,
    result, progress, progressMessage,
  ...
  const headerLabel = lockedOp
    ? (OP_LABELS[lockedOp] ?? lockedOp)
